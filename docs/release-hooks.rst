...
The :doc:`/commands/release` command makes a new release for the current branch.
...
Now, instead of doing this manually, HuPKit allows you to hook-into the release process,
executing a custom callback before and/or after a new release is created.

## The Script

Add a PHP  script named either ``pre-release.php`` or ``post-release.php`` at the root folder
of the ``_hubkit`` :ref:`configuration branch <repository-configuration>`.
...
.. caution::
    ...
    The hook is executed in the HuPKit application's context, you have
...
In this case it might be better to use the ``process`` service to execute
a separate command.

.. tip::

    HuPKit v2.0 will contain a complete plug-in architecture with
    more flexibility and dependency handling.
...
.. important::
    ...
    This technique is not to be used as-is, understand the risk and be
    sure to apply enough error protections.
